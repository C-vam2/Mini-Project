...
namespace my
{
        ...
        FarnebackPolyExp(const Mat& src, Mat& dst, int n, double sigma)
    {
        ...
        for (y = 0; y < height; y++)
        {
            ...
            for (k = 1; k <= n; k++)
            {
                ...
                srow0 = (float*)(src.data + src.step * max(y - k, 0));
                srow1 = (float*)(src.data + src.step * min(y + k, height - 1));
            ...
            bool update_matrices)
    {
        ...
        for (y = 0; y < height; y++)
        {
            ...
            for (i = 0; i <= m; i++)
            {
                srow[m - i] = (const float*)(matM.data + matM.step * max(y - i, 0));
                srow[m + i] = (const float*)(matM.data + matM.step * min(y + i, height - 1));
            }
        ...
        std::vector<float>& fscales, int poly_n, double poly_sigma)
    {
        ...
        for (int k = 0; k < poly_exp_pyr.size(); k++)
        {
            ...
            resize(fimg, I, Size(width, height));
