...
void BuildDescMat(const Mat& xComp, const Mat& yComp, float* desc, const DescInfo& descInfo)
{
	...
	int cntr = 0,cntr2=0;
	int totalCells = (xComp.cols * xComp.rows);
	...
	//cout << "\nCounter== " << cntr << " " << totalCells << " " << cntr2 << endl;
}
...
{
	int dim = descInfo.dim;    // nBins * nxycell * nxycell
	int nBins = descInfo.nBins;
	int height = descMat->height;  // same as height of frame
	int width = descMat->width;    // same as width of frame

	int xStride = rect.width / descInfo.nxCells;   //16 = 32 / 2
	int yStride = rect.height / descInfo.nyCells;  // 16 = 32 / 2
	int xStep = xStride * nBins;                   
	int yStep = yStride * width * nBins;

	//cout << dim << "   " << nBins << "  " << height << "  " << width << "  " << xStride << "  " << yStride << " " << xStep << "  " << yStep << endl;
...
/*this function evaluates the validity of a trajectory based on its mean, variance, maximum displacement, and length,
and returns true if it passes the checks and false otherwise.*/
...
/* we can remove trajectories generated by cameramotion. This can be achieved by thresholding the displacement vectors of the trajectories 
in the warped flow field. Ifthe displacement is too small, the trajectory is consideredto be too similar to camera motion, and thus removed*/
...
void DrawTrack(const std::vector<Point2f>& point, const int index, const float scale, Mat& image,bool flag)
{
	...
	for (int j = 1; j <= index; j++)
	{
		...
		point1 *= scale;
		if(flag)
			line(image, point0, point1, Scalar(cvFloor(255.0 * (j + 1.0) / float(index + 1.0)), cvFloor(255.0 * (j + 1.0) / float(index + 1.0)), cvFloor(255.0 * (j + 1.0) / float(index + 1.0))), 1, 8, 0);
		else
			line(image, point0, point1, Scalar(0, cvFloor(255.0 * (j + 1.0) / float(index + 1.0)), 0), 1, 8, 0);
		point0 = point1;
	}
	circle(image, point0, 1, Scalar(0, 0, 255), -1, 8, 0);
}

void PrintDesc(std::vector<float>& desc, DescInfo& descInfo, TrackInfo& trackInfo,std::vector<float>& container)
{
	//printf("\n%d %d %d\n", descInfo.nxCells, descInfo.nyCells, descInfo.ntCells);
	...
	for (int i = 0; i < descInfo.ntCells; i++)
	{
		...
		for (int j = 0; j < dim; j++) {
			...
			container.push_back(0.7*vec[j] * norm);
		}
	}
}
...
cv::Mat windowedMatchingMask(const vector<cv::KeyPoint>& keypoints1, const vector<cv::KeyPoint>& keypoints2,float maxDeltaX, float maxDeltaY)
{
	if (keypoints1.empty() || keypoints2.empty())
		return cv::Mat();

	int n1 = (int)keypoints1.size(), n2 = static_cast<int>(keypoints2.size());
